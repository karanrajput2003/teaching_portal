...
if(isset($_REQUEST['addquizbtn']))
{
     ...
     || ($_REQUEST['correct_option'] == ""))
    {
        ...
        $sql = "INSERT INTO quiz_questions (lesson_id, course_id, question_name, option_A, option_B, option_C, option_D, correct_option) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

if ($stmt = $conn->prepare($sql)) {
    $stmt->bind_param("iissssss", $lesson_id, $course_id, $question_name, $option_A, $option_B, $option_C, $option_D, $correct_option);


    if ($stmt->execute()) {
        $msg = '<div class="text-success">Added Successfully</div>';
    } else {
        $msg = '<div class="text-danger">Error: ' . $stmt->error . '</div>';
    }

    $stmt->close();
} else {
    $msg = '<div class="text-danger">Error: ' . $conn->error . '</div>';
}
